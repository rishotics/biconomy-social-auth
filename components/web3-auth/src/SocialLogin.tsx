...
class SocialLogin {
  ...
  async bananaLogin() {
    const bananaLogin = new Banana(Chains.mumbai);
      const bananaWallet = await bananaLogin.createWallet();
      const bananaSigner = await bananaWallet.getSigner();
      const signMessageResponse = await bananaSigner.signBananaMessage("sampleMsg");
      console.log(JSON.stringify(signMessageResponse));
      const web3authProvider = await bananaWallet.getProvider();
      return bananaSigner;
  }
